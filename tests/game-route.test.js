...
describe('Play Route', () => {
  ...
  it('should test if the ship player id is equal to the user id, meaning they are associated', async() => {
    ...
    const shipInfoA = await Ship.create( {name: 'A', size: 5, location:['d7'], player: computer._id,
    });
  ...
  it('should test if the shot is correctly placed on the users target board', async() => {
    ...
    const user = await User.create( {username: 'Computer', password: 'pass',
    });

    const shipInfoA = await Ship.create( {name: 'A', size: 5, location:[], player: user._id,
    });

    const primaryBoardInfo = await Board.create({type: 'primary', player: user._id, board: {
      a: ['~', '~', '~', '~', '~', '~', '~', '~', '~', '~'],
      b: ['~', '~', '~', '~', '~', '~', '~', '~', '~', '~'],
      c: ['~', '~', '~', '~', '~', '~', '~', '~', '~', '~'],
      d: ['~', '~', '~', '~', '~', '~', '~', '~', '~', '~'],
    ...
    const trackingBoardInfo = await Board.create({type: 'tracking', player: user._id, board: {
      a: ['-', '-', '-', '-', '-', '-', '-', '-', '-', '-'],
      b: ['-', '-', '-', '-', '-', '-', '-', '-', '-', '-'],
      c: ['-', '-', '-', '-', '-', '-', '-', '-', '-', '-'],
      d: ['-', '-', '-', '-', '-', '-', '-', '-', '-', '-'],
